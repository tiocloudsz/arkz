...
module.exports = conn = async (conn, m, chatUpdate, store) => {
  try {
    ...
    const getCase = (cases) => {
      return ("case  " + `'${cases}'` + fs.readFileSync("./Arifzyn.js").toString().split("case '" + cases + "'")[1].split("break")[0] + "break");
    };
    const totalFitur = () => {
      var mytext = fs.readFileSync("./Arifzyn.js").toString();
    ...
    // Function Created By ArifzynXD Alias Dryan ft .𝐗𝐓𝐑𝐀𝐌
...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`;
      ...
      case "attp":
        ...
        try {
          if (args.length == 0)
            return newReply(`Example: ${prefix + command} ArifzynXD`);
      ...
      case "tomp3":
        {
          ...
          conn.sendMessage(
            m.chat,
            {
              document: audio,
              mimetype: "audio/mpeg",
              fileName: `Convert By ArifzynXD.mp3`,
            },
            { quoted: m }
          );
        }
...
◇ ‣ ${prefix}memeindo`;
    ...
    console.log("Eror Di Bagian Arifzyn.js " + util.format(err));
  }
};
